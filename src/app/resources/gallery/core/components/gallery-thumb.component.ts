...
@Component({
  ...
  template: `
    <div class="headerGallery py-3 ml-2 mr-8"><span [class.ml-7]="!hide" [class.ml-1]="hide"> Previous segments</span></div>
  ...
  thumbsStyle(delta: number) {
    ...
    switch (this.config.thumbPosition) {
      ...
      case 'right':
        ...
        value = -(this.state.currIndex * this.config.thumbHeight) - (this.config.thumbHeight / 6 - delta);
        return {
